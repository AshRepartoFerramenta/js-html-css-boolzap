$(document).ready(function () {
  ...
  $("#add-message").keypress(function (event) {
  ...
  $(document).on("click", ".item_wrapper", function () {
    ...
    var pannelloConversazione = $(
      '.conversation[data-contact="' + conversazione + '"]'
    );
    ...
    $(".main_content_right .current_chat .current_chat_name .head_name").text(
      name
    );
    $(".main_content_right .current_chat .current_chat_name .head_time").text(
      "Ultimo accesso oggi alle" + " " + time
    );
    $(".main_content_right .current_chat .current_chat_avatar img").attr(
      "src",
      img
    );
  });
...
function receivedMessage() {
  var textMessage = "Sei Unico AshRepartoGrandiMagazini!";
  var newMessage = $(".template .message").clone();
  newMessage.find(".message-text").text(textMessage);
